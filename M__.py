...
kl = LINETCR.LINE()
kl.login(token="EoXDpkVpQQUEWoXM7DF2.+mqFDynoxhlr0ICWnX/ZaG.yyae2B/H+LSFcNI8Y15W6y/iMWYiTvRvM1qNnzDlZK0=")
kl.loginResult()
...
KAC=[cl,ki,kk,kc,kd,ke,kf,kg,kh,kl,km]
...
autoleaveroom = []
targets = []
Bots = ["u04ed6417e1922a439e5b25d2d15ed950""ua1cb6e845fe8f2646fe8a5c5911841fa",mid,Amid,Bmid,Cmid,Dmid,Emid,Fmid,Gmid,Hmid,Jmid,Lmid,Mmid]
self = ["u04ed6417e1922a439e5b25d2d15ed950""ua1cb6e845fe8f2646fe8a5c5911841fa",mid,Amid,Bmid,Cmid,Dmid,Emid,Fmid,Gmid,Hmid,Jmid,Lmid,Mmid]
...
#-----------------------------------------------
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention"] == True:
                     ...
                     balas = [cName + " หำอย่าแทคเยอะสิ?, ", cName + "หำกูบอกว่าอย่าแทคเยอะ " + cName + "?", "มีอะไรหรอครับ, " + cName + "?", "ถามจริงมีอะไรหรอ?", "คุณเรียกผมใช่ไม", cName + "โอ้ยยยย!!!!!ถ้าไม่ไหวจริงๆหลังตลาดนู้น"]
                     ret_ = "[Auto Respond]\n" + random.choice(balas)
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["kickMention"] == True:
                     contact = cl.getContact(msg.from_)
                     cName = contact.displayName
                     balas = ["",cName + " อย่าแทคกูหำ?, ", cName + "หลังไมค์ก็ได้มั้ง" + cName + "?", "เห้ยยนี้เพื่อนใครว่ะ, " + cName + "?","เอ่อๆรู้แล้วกำลังมา!!!!. ", "ใจเย็นๆสิแทคอยู่ได้"]
                     ret_ = "[Auto Respond]\n" + random.choice(balas)
                     name = re.findall(r'@(\w+)', msg.text)
                     mention = ast.literal_eval(msg.contentMetadata['MENTION'])
                     mentionees = mention['MENTIONEES']
